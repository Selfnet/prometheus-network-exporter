...
class InterfaceCollector(Collector):
    default = yaml.load(read_text(junos, "interface.yaml"), Loader=yaml.SafeLoader)
    name = "interface"
    ...
    def collect(self):
        ...
        interfaces_dict = (
            self.device.get_interfaces(interface_regex=self.interface_regex)
            if self.access
            else self.device.get_interfaces()
        )
        ...
        for prometheus in self.prometheus_metrics.values():
            ...
            for interface in interfaces_list:
                prometheus.metric.add_metric(
                    ...
                    value=interface.get(prometheus.json_key),
                )
