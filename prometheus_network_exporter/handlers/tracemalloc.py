...
class TraceMallocHandler(tornado.web.RequestHandler):
    ...
    def display_top(self, snapshot, key_type='lineno', limit=10):
        ...
        for index, stat in enumerate(top_stats[:limit], 1):
            ...
            self.write(bytes(("#%s: %s:%s: %.1f KiB\n" % (index, filename, frame.lineno, stat.size / 1024)).encode('utf-8')))
        ...
        if other:
            ...
            self.write(bytes(("%s other: %.1f KiB\n" % (len(other), size / 1024)).encode('utf-8')))
        total = sum(stat.size for stat in top_stats)
        self.write(bytes(("Total allocated size: %.1f KiB\n" % (total / 1024)).encode('utf-8')))
